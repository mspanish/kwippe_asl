...
//http://jsfiddle.net/21sLk84f/
    ...
    if (changedAngle) {
        ...
        $settings.linearAngle = angle;
    }
    else {
        angle = $settings.linearAngle;
     }
    ...
    if ($settings.gradient == 'radial') {
        ...
        // for user manipulating the size of the gradient:
    
        size.width = size.width + $settings.gradientWidthOffset;
        size.height = size.height + $settings.gradientHeightOffset;
        if (size.width < 0) size.width = 0;
        if (size.height < -300) size.height = 0;
...
<div class="bottomSlide" style="margin-top:10px;">
    ...
    <span  class="slideTitle">Add a Gradient</span>
        
    <div class="section" id="add_a_gradient"> 
        ...
        {#if $settings.gradient == 'linear'} 
            <div> 
                <p>set angle manually:</p>
                <input id="linear_angle" bind:value={$settings.linearAngle} step="10" min="-90" max="180" type="range" on:input="{setGradient}">  
            </div>
            

            <p>or pick a direction</p>
            <div class="inline">
                <input name="gradient_dir" type="radio" on:input="{angleUpdate}" value="up" checked="{$settings.gradientDirection == 'up' ? 'checked' : ''}" id="up">
                <label for="up">up</label>
            </div>    
            <div class="inline">
                <input name="gradient_dir" type="radio" on:input="{angleUpdate}" value="down" checked="{$settings.gradientDirection == 'down' ? 'checked' : ''}" id="down">
                <label for="down">down</label>
            </div>  
            <div class="inline">
                <input name="gradient_dir" type="radio" on:input="{angleUpdate}" value="left" checked="{$settings.gradientDirection == 'left' ? 'checked' : ''}" id="left">
                <label for="left">left</label>
            </div>          
            <div class="inline">
                <input name="gradient_dir" type="radio" on:input="{angleUpdate}" value="right" checked="{$settings.gradientDirection == 'right' ? 'checked' : ''}" id="right">
        ...
        {#if $settings.gradient == 'radial'} 
            <div> 
                ...
                <input id="width_offset" bind:value={$settings.gradientWidthOffset} step="1" min="-500" max="500" type="range" on:input="{setGradient}">  
                <span>move right/left:</span>

                <input id="height_offset" bind:value={$settings.gradientHeightOffset} step="1" min="-500" max="500" type="range" on:input="{setGradient}">  
